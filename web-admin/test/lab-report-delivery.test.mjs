...
test("lab report pdf metadata includes approval, publication, verification, and signature fields", () => {
  const source = readSource("pages/lab/LabPage.tsx");
  const repoRoot = fs.existsSync(path.join(process.cwd(), "backend")) ? process.cwd() : path.resolve(process.cwd(), "..");
  ...
  assert.ok(backend.includes("Authorized Signatory"));
  assert.ok(backend.includes("Verification Reference"));
  assert.ok(backend.includes("reportVerificationToken"));
  assert.ok(backend.includes("Digitally verified"));
