...
test("doctor profile edit payload and labels keep dob-based validation", () => {
  ...
  assert.ok(detailSource.includes("SPECIALIZATION_OPTIONS"));
  assert.ok(detailSource.includes("QUALIFICATION_OPTIONS"));
  assert.ok(detailSource.includes("Other specialization"));
  assert.ok(detailSource.includes("Other qualification"));
  assert.ok(detailSource.includes("normalizeSpecializations(form.specializations, form.specializationsInput)"));
  assert.ok(detailSource.includes("parseQualificationValues(profile.qualification)"));
  assert.ok(detailSource.includes("normalizeQualificationValues(form.qualifications, form.qualificationOther)"));
  ...
  assert.ok(detailSource.includes("sanitizeMoneyInput(e.target.value)"));
  ...
  assert.ok(detailSource.includes("sanitizeIndianMobileInput(e.target.value)"));
  assert.ok(detailSource.includes('inputMode: "numeric"'));
  assert.ok(detailSource.includes('inputMode: "decimal"'));
  assert.ok(detailSource.includes("maxLength: 10"));
  assert.ok(detailSource.includes("multiple"));
  assert.ok(detailSource.includes("qualifications: qualification.qualifications"));
  assert.ok(detailSource.includes("Age (derived)"));
  assert.ok(detailSource.includes("Date of Birth"));
  assert.ok(detailSource.includes("RequiredLabel text=\"Mobile\" />"));
  assert.ok(detailSource.includes("RequiredLabel text=\"Specialization\" />"));
  assert.ok(detailSource.includes("RequiredLabel text=\"Qualification\" />"));
  assert.ok(detailSource.includes("RequiredLabel text=\"Registration Number\" />"));
  assert.ok(detailSource.includes("RequiredLabel text=\"OPD Fee\" />"));
  assert.ok(detailSource.includes("RequiredLabel text=\"Follow-up Fee\" />"));
  assert.ok(detailSource.includes("RequiredLabel text=\"Emergency Fee\" />"));
  assert.ok(detailSource.includes("RequiredLabel text=\"Years of Experience\" />"));
  assert.ok(detailSource.includes("RequiredLabel text=\"Date of Birth\" />"));
