...
test("auth provider surfaces expired-session messaging and token-expired refresh handling", () => {
  ...
  assert.ok(authProvider.includes('resetAuthState(SESSION_EXPIRED_MESSAGE, true)'));
  ...
  assert.ok(authProvider.includes('broadcastLogout("inactivity")'));
  assert.ok(authProvider.includes('void terminateKeycloakSession("inactivity");'));
