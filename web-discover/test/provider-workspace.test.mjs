...
test("provider workspace uses canonical provider routes and business-facing account text", () => {
  ...
  assert.ok(page.includes("Manage your applications, published profiles, and account access."));
  assert.ok(page.includes("Total applications"));
  assert.ok(page.includes("Draft applications"));
  assert.ok(page.includes("Submitted applications"));
