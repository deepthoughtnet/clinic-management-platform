...
export function buildPatientPortalVoiceWebSocketUrl(session: PatientPortalPatientSession) {
  ...
  wsUrl.pathname = basePath.endsWith("/api") ? `${basePath.slice(0, -4)}/ws/patient-portal/careai` : "/ws/patient-portal/careai";
  wsUrl.hash = "";
  wsUrl.search = "";
  wsUrl.searchParams.set("sessionToken", session.patientSessionToken);
