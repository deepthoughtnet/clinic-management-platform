...
class CommercialPlatformMigrationTest extends PostgresTestContainerSupport {

    @Test
    void migrationCreatesPlanTablesAndAppliesCleanly() throws Exception {
        try (ManagedSchema schema = createSchema()) {
            ...
            try (Connection connection = connection()) {
                assertThat(tableExists(connection, schema.name(), "commercial_plan_templates")).isTrue();
                assertThat(tableExists(connection, schema.name(), "commercial_plan_drafts")).isTrue();
                assertThat(tableExists(connection, schema.name(), "commercial_plan_versions")).isTrue();
                assertThat(countRows(connection, schema.name(), "commercial_plan_templates")).isZero();
    ...
    private static ManagedSchema createSchema() throws Exception {
        String schema = "commercial_platform_" + UUID.randomUUID().toString().replace("-", "");
        try (Connection connection = connection();
