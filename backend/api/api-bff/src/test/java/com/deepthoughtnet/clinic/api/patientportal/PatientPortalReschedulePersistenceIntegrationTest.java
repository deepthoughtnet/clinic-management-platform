...
class PatientPortalReschedulePersistenceIntegrationTest extends PostgresTestContainerSupport {
    ...
    @Test
    void ownerAwareRescheduleCommitsAndFreshNetworkReadShowsNewSchedule() {
        ...
        OffsetDateTime[] originalUpdatedAt = new OffsetDateTime[1];
        ...
        transactionTemplate.executeWithoutResult(status -> {
            ...
            AppUserEntity currentUser = AppUserEntity.create(currentTenant, "portal-" + UUID.randomUUID(),
                    "portal@example.com", "Portal Patient");
            currentUser.setPatientId(currentPatient.getId());
            AppUserEntity doctor = AppUserEntity.create(ownerTenant, "doctor-" + UUID.randomUUID(),
                    "doctor@example.com", "Owner Doctor");
            ...
            DoctorAvailabilityEntity availability = DoctorAvailabilityEntity.create(ownerTenant, doctor.getId());
            availability.update(targetDate.getDayOfWeek(), LocalTime.of(9, 0), LocalTime.of(10, 0),
                    null, null, 30, 1, true);
            availabilityRepository.save(availability);

            AppointmentEntity appointment = AppointmentEntity.create(ownerTenant, ownerPatient.getId(), doctor.getId());
            appointment.update(sourceDate, sourceTime, null, "Review", AppointmentType.SCHEDULED,
                    AppointmentStatus.BOOKED, null);
            appointmentRepository.save(appointment);
            appointmentId[0] = appointment.getId();
            originalUpdatedAt[0] = appointment.getUpdatedAt();
        });
        ...
        RequestContextHolder.set(new RequestContext(TenantId.of(currentTenant), currentUserId[0], "portal-sub",
                Set.of("PATIENT"), "PATIENT", "reschedule-persistence-test"));

        var confirmation = patientPortalService.rescheduleAppointmentInOwningTenant(
                appointmentId[0], ownerTenant, targetDate, targetTime, "Rescheduled by patient",
                "integration-reschedule-" + UUID.randomUUID());

        assertThat(confirmation.status()).isEqualTo("BOOKED");
        entityManager.clear();
        var persisted = transactionTemplate.execute(status -> appointmentRepository
                .findByTenantIdAndId(ownerTenant, appointmentId[0]).orElseThrow());
        assertThat(persisted.getStatus()).isEqualTo(AppointmentStatus.BOOKED);
        assertThat(persisted.getAppointmentDate()).isEqualTo(targetDate);
        assertThat(persisted.getAppointmentTime()).isEqualTo(targetTime);
        assertThat(persisted.getVersion()).isEqualTo(1);
        assertThat(persisted.getUpdatedAt()).isAfter(originalUpdatedAt[0]);

        var upcoming = transactionTemplate.execute(status -> patientPortalService.careAiUpcomingAppointmentsAcrossAuthorizedClinics());
        assertThat(upcoming).anyMatch(row -> appointmentId[0].equals(row.appointmentId())
                && targetDate.equals(row.appointmentDate()) && targetTime.equals(row.appointmentTime()));
        assertThat(upcoming).noneMatch(row -> appointmentId[0].equals(row.appointmentId())
                && sourceDate.equals(row.appointmentDate()) && sourceTime.equals(row.appointmentTime()));
